...
from src.pipeline.runner import generate_recommendations, save_evaluation_results
...
class ModelTuningSpec:
    ...
    Tune grid expander iterates over.
    """
...
def tune_model(
    ...
    return _materialize_config(spec, dict(best_hyperparameters))


def _evaluate_best_config_on_validation(
    spec: ModelTuningSpec,
    config: ModelConfig,
    context: ValidationContext,
) -> list[EvaluationResult]:
    """Re-train and re-evaluate the best config on each validation split for CSV reporting."""
    model = _build_model(spec, config, context)
    needs_sequences = bool(context.validation_sequences)
...
def run_tuning_pipeline(
    ...
    try:
        for spec in specs:
            ...
            best_configs[spec.model_name] = tune_model(
                spec, spec_context, use_gpu=use_gpu
            )
    finally:
        ray.shutdown()
    ...
    if results_directory is not None:
        print("\nRe-evaluating best configs on validation splits...")
        for spec in specs:
            spec_context = _filter_context_for_spec(context, spec)
            results = _evaluate_best_config_on_validation(
                spec, best_configs[spec.model_name], spec_context
            )
            save_evaluation_results(results, results_directory, stage="tuning")
