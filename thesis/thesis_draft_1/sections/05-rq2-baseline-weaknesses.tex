...
We begin by comparing the aggregate performance of the two baselines at their primary-metric optima, summarised in Table~\ref{tab:baseline-aggregate}:
\begin{itemize}
...
\begin{tabular}{lrrr}
\toprule
Model & ROI@10 & PC@10 & PC-lift@10 \\
\midrule
LightGCN & $-0.005$ & $0.794$ & $1.17\times$ \\
RF       & $+0.014$ & $0.667$ & $1.06\times$ \\
\bottomrule
\end{tabular}
...
However, aggregate PC@10 averages across customer bands whose random baselines $\pi(b)$ vary from 0.35 to 0.78, so a single number can mask which bands are actually being served. Table~\ref{tab:baseline-per-band} decomposes PC@10 by declared band to expose these differences:
\begin{itemize}
...
Aggressive's high lift is partly mechanical as $\pi(\text{Aggressive})$ is the lowest band of 0.35, so any model skewing higher-volatility clears that bar easily. Neither baseline has band labels or a band-aware loss, so per-band coherence is a passive byproduct of training rather than a controlled output.
...
\label{tab:baseline-per-band}
\end{table}

\subsection{Limitations}
\label{sec:rq2:limitations}

Two caveats bound the RQ2 audit.

\begin{itemize}
    \item \textit{Single operating point.} The audit evaluates each baseline only at its primary-metric optimum (nDCG@10 for LightGCN, ROI@10 for Random Forest), so the per-band picture is conditioned on those operating points rather than swept across the full grid.
    \item \textit{Baseline-relative reading.} Per-band lift should be read alongside its $\pi(b)$ reference rather than in isolation, since low-$\pi(b)$ bands such as Aggressive clear the bar more easily.
\end{itemize}
