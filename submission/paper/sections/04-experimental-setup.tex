...
Table~\ref{tab:baseline-grids} reports the search grids and fixed values for the two FAR-Trans baselines. Both grids are swept with Ray Tune, with LightGCN tuned on average nDCG@10 and Random Forest on average ROI@10 across the 69 splits. Best-trial values are reported in the appendix.
...
\end{split}
\end{equation}
where $r = (p_{\text{end}} - p_{\text{start}})/p_{\text{start}}$ is the 6-month realised return, trade-date and end prices are matched by backward-asof (7-day tolerance, rows over a week dropped), and \texttt{is\_coherent} is binary.
